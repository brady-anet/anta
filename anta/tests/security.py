...
class VerifySSHFIPSRestrictions(AntaTest):
    ...
    Expected Results
    ----------------
    * Success: The test will pass if ``FIPS status: enabled`` is present in the ``show management ssh`` output.
    * Failure: The test will fail if the FIPS status line is absent or reports ``FIPS status: disabled``.
